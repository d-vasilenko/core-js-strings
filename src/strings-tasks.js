...
function removeLastOccurrences(str, value) {
  ...
  const leftSide = str.slice(0, index);
  const rightSide = str.slice(index + value.length);
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    sumOfCharacters += str.charCodeAt(i);
  }
...
function countVowels(str) {
  ...
  Array.from(str).forEach((item) => {
    if (vowels.includes(item)) counter += 1;
  });
  return counter;
}
/**
...
function isPalindrome(str) {
  ...
  return (
    newStr.toLowerCase().replaceAll(' ', '') ===
    newStr.toLowerCase().replaceAll(' ', '').split('').reverse().join('')
  );
}
/**
...
function extractNameFromTemplate(value) {
  ...
  const lsName = value.split(' ')[2].slice(0, -1);
  return `${name} ${lsName}`;
}
